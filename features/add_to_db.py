...
def add_to_all_operations(date, time, metal, quantity, price, amount, comment):
    ...
    all_operations.date = datetime.datetime.strptime(date, "%Y.%m.%d").date()  # Записываем дату
    all_operations.time = datetime.datetime.strptime(time, "%H:%M").time()  # Записываем время
